...
/*
    16-bit  WRITE 0x60C0 0110_0000_1100_0000
    X           0
    DAEMCL      1   Clear DAEMCL interrupt
    ADFLCL      1   clear ADC FIFO Full latch
    DAEMIE      0   DAC FIFO Empty signal interrupt
    X           0
    X           0
    X           0
    X           0
    INTCL       1   clear INT[1:0] interrupt
    EOACL       1   clear EOA interrupt
    DAHFCL      0   Clear DAC FIFO Half-Full interrupt (disabled)
    EOAIE       0   End of acquisition interrupt (disabled) 
    DAHFIE      0   Used for DAC operation (disabled)
    INTE        0   disable INT1 INT0
    INT1        0   External (disabled by INTE)
    INT0        0
*/
#define	MUXCHAN			iobase[1] + 2				// Badr1 + 2
/*
    16-bit WRITE 0x0D00 READ 0x4000
    X           0
    X           0
    ADPS1       0   Clock (SW Convert) ie. software clock
    ADPS0       0   
    UNIBIP      1   Unipolar 0-5V, input gain = 2     NOTE: Bipolar is +-5V
    SEDIFF      1   Single Ended / Differential (Single ended)
    GS1         0   ADC range of GS[0,1] = 5V
    GS0         1
    CHH8        0
    CHH4        0
    CHH2        0
    ...
    READ only concerns the 14th bit
    EOC         1  ADC_DONE
*/
...
/*
    ...
    X           0
    C0SRC       1   10Mhz clock
    FFM0        0   Dont understand
    ARM         0
    HMODE       0   negative hysterisis
    CHI_EN      0
    CLO_EN      0   
    XTRCL       1   Clear xtrig
    PRTRG       0   Disable Pre-trigger Mode
    BURSTE      0   Burst Mode disabled
    TGEN        0   trigger source has no effect NOTE: so TS is useless??
    TGSEL       0   LEVEL triggered
    TGPOL       0   external trigger input not inverted
    TS1         0   SW_Trigger
    TS0         1
...
/*
    16-bit 0x007f
    SDI         0   irrelevant
    CALEN       0   ADC0 function as normal input
    CASRC2      0   AGND
    CASRC1      0
    CASRC0      0   
    SEl08       0   offset DAC conversion disabled
    SEL8402     0   ADC Gain Cal
    SEL8800     0   DAC0 File Gain
    CD7         0
    CD6         1
    CD5         1
    CD4         1
...
/*
    16-bit WRITE 0x0a23 0x0a43
    X       0
    X       0
    X       0
    X       0
    DAC1R1  1   unipolar 5V
    DAC1R0  0
    DAC0R1  1   unipolar 5V
    DAC0R0  0
    X       0
    HS1     0   DAC0    1  DAC1
    HS0     1           0  
    DAPS1   0   SW convert
    DAPS0   0   
    START   0   disable FIFO'd DAC operation
    DACEN   1   enable DAC0 and DAc1
    LDAEMCL 1   Reset Empty flag
*/

#define 	AD_DATA			iobase[2] + 0				// Badr2 + 0
/*
    16-bit      conversion status determined by polling EOC until '1'
*/
#define 	AD_FIFOCLR		iobase[2] + 2				// Badr2 + 2
/*
    write to this clears FIFO, data dont care
*/
